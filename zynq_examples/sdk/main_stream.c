...
XBasic_stream xbasic_dev;
XBasic_stream_Config xbasic_config = {
	0,
	XPAR_BASIC_STREAM_S_AXI_CTRL_BUS_BASEADDR
};
int XBasicSetup(){
  return XBasic_stream_CfgInitialize(&xbasic_dev,&xbasic_config);
}
void XBasicStart(void *InstancePtr){
  XBasic_stream *pExample = (XBasic_stream *)InstancePtr;
  XBasic_stream_InterruptEnable(pExample,1);
  XBasic_stream_InterruptGlobalEnable(pExample);
  XBasic_stream_Start(pExample);
}
void XBasicIsr(void *InstancePtr){
  XBasic_stream *pExample = (XBasic_stream *)InstancePtr;
  XBasic_stream_InterruptGlobalDisable(pExample);
  XBasic_stream_InterruptDisable(pExample,0xffffffff);
  XBasic_stream_InterruptClear(pExample,1);
  ResultExample = 1;
  if(RunExample){XBasic_stream_Start(pExample);}
}
int XBasicSetupInterrupt() {
  ...
  Xil_ExceptionEnable();
  result = XScuGic_Connect(&ScuGic,XPAR_FABRIC_BASIC_STREAM_INTERRUPT_INTR,(Xil_InterruptHandler)XBasicIsr,&xbasic_dev);
  if(result != XST_SUCCESS){return result;}
  XScuGic_Enable(&ScuGic,XPAR_FABRIC_BASIC_STREAM_INTERRUPT_INTR);
  return XST_SUCCESS;
}
void init_timer() { 
