...
## compilation (algo)
DEPFILE=../firmware/cfg/mp7_regionizer.dep
test -f $DEPFILE || exit 4;
for F in $(awk '/^src/{print $2}' $DEPFILE); do
    xvhdl $HERE/../firmware/hdl/$F || exit 4;
    grep -q ERROR xvhdl.log && exit 4;
done;
## compilation (testbench)
DEPFILE=cfg/testbench.dep
test -f $DEPFILE || exit 4;
for F in $(awk '/^src/{print $2}' $DEPFILE) $TESTBASE; do
